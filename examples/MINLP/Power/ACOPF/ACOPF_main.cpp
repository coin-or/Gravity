...
////    PAD_UB -= grid->tbound_max_tan.in(grid->arcs)*(vr.from(grid->arcs)*vr.to(grid->arcs) - vi.to(grid->arcs)*vi.from(grid->arcs));
////    ACOPF.add_constraint(PAD_UB <= 0);
////
////    Constraint PAD_LB("PAD_LB:");
////    PAD_LB = vr.from(grid->arcs)*vi.to(grid->arcs) + vr.to(grid->arcs)*vi.from(grid->arcs);
////    PAD_LB -= grid->tbound_min_tan.in(grid->arcs)*(vr.from(grid->arcs)*vr.to(grid->arcs) - vi.to(grid->arcs)*vi.from(grid->arcs));
////    ACOPF.add_constraint(PAD_LB >= 0);
 ...
 //   OPF.run();
    ...
    //solver OPF(SOCP,cplex);
    solver OPF(SOCP,ipopt);
    OPF.run();
    return 0;
}
