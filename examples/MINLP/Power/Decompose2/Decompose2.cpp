...
void reform_inout (PowerNet& grid, unsigned nbparts) {
    ...
    for (int c = 0; c < nbparts; c++) {
        ...
        CLT.add_var(bag_Xii^(P.bag_bus_union[c].size()));
        ...
        CLT.add_var(bag_R_Xij^(P.bag_bus_pairs_union[c].size()));
        CLT.add_var(bag_Im_Xij^(P.bag_bus_pairs_union[c].size()));
        ...
        if (P.bag_gens[c].size() > 0) {
            ...
            CLT.add_var(bag_Pg^(P.bag_gens[c].size()));
            CLT.add_var(bag_Qg^(P.bag_gens[c].size()));
            Pg.push_back(bag_Pg);
            Qg.push_back(bag_Qg);
        }
        else {
