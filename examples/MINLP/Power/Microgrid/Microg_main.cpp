...
//  Gravity
//
//  Created by Hassan Hijazi on Dec 06 2017
//
//
#include <stdio.h>
#include <iostream>
...
int main (int argc, const char * argv[])
{
    ...
    if (argc >= 2) {
        ...
        fname = "../../data_sets/Power/Dercam.xlsx";
    }
    PowerNet grid;
    double vlb, vub, kvb, pl, ql;
    unsigned index, time, b_type;
    string name;
    xlnt::workbook wb;
    double wall0 = get_wall_time();
    double cpu0  = get_cpu_time();
    cout << "Reading all excel sheets...\n";
    wb.load(fname);
    ...
    std::clog << "Processing PowerFlowParams..." << std::endl;
    auto row_it = ws.rows().begin();
    auto row = *row_it++;
    name = row[1].to_string();
    DebugOn("Ref Bus name = " << name << endl);
    grid.ref_bus = name;
    while (row_it!=ws.rows().end())
    {
        ...
        if (name.compare("Sbase")==0) {
            grid.bMVA = row[1].value<double>();
            DebugOn("Base MVA = " << to_string(grid.bMVA) << endl);
        }
        if (name.compare("Vbase")==0) {
            grid.bV = row[1].value<double>();
            DebugOn("Base V = " << to_string(grid.bV) << endl);
        }
        if (name.compare("NoOfNodes")==0) {
            grid.nb_nodes = row[1].value<double>();
            DebugOn("Number of buses = " << to_string(grid.nb_nodes) << endl);
            for (size_t i = 0; i<grid.nb_nodes; i++) {
                auto bus = new Bus(to_string(i));
                grid.add_node(bus);
            }
        }
    }
    vector<tuple<double,double,double>> cableparams;
    ws = wb.sheet_by_title("CableParams");
    std::clog << "Processing CableParams..." << std::endl;
    row_it = ws.rows().begin();
    row_it++;//SKIP FRIST ROW
    while (row_it!=ws.rows().end())
    {
        auto row = *row_it++;
        cableparams.push_back(make_tuple(row[4].value<double>(), row[5].value<double>(), row[7].value<double>()));
    }
    ws = wb.sheet_by_title("BranchType_Cable");
    std::clog << "Processing BranchType_Cable..." << std::endl;
    row_it = ws.rows().begin();
    row_it++;//SKIP FRIST ROW
    index = 0;
    for (size_t i = 0; i<grid.nb_nodes-1; i++) {
        auto row = *row_it++;
        for (size_t j = i+1; j<grid.nb_nodes; j++) {
            if (row[j+1].has_value()) {
                b_type = row[j+1].value<unsigned>();
                DebugOn("(" << i+1 << "," << j+1 << ") = type " << b_type << endl);
                auto arc = new Line(to_string(index) + "," + to_string(i+1) + "," + to_string(j+1));
                arc->_id = index++;
                arc->_src = grid.get_node(to_string(i));
                arc->_dest= grid.get_node(to_string(j));
                arc->b_type = b_type;
                arc->r = get<0>(cableparams[b_type-1]);
                arc->x = get<1>(cableparams[b_type-1]);
                arc->limit = get<2>(cableparams[b_type-1]);
                arc->connect();
                grid.add_arc(arc);
                arc->print();
            }
    ...
    //            continue;
    //        }
    //        ws = wb.sheet_by_title("Pt_"+to_string(b->_id+1));
    //        std::clog << "Processing Pt_"+to_string(b->_id) << std::endl;
    //        //    for (auto row : ws.rows(false))
    //        DebugOn("Bus name = " << name << endl);
    //
    //        time = 0;
    //        auto row_it = ws.rows().begin();
    //        while (row_it!=ws.rows().end())
    //        {
    //            auto row = *(row_it++);
    //            pl = row[1].value<double>();
    //            grid.pl.set_val(b->_id, time,pl);
    //            DebugOn("pl at time" << time << "  = " << pl << ", ");
    //            time++;
    //        }
    //        DebugOn(endl);
    //    }
