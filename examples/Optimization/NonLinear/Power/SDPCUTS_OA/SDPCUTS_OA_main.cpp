...
//        }
    ...
    UB_solver.run(output = 5, 1e-6,"ma27");
    if(OPF->_status!=0){
    ...
    /** Build model */
    auto SDP = make_shared<Model<>>("SDP-OPF Model");
    ...
    var<int> dummy("dummy", -1,1);
    SDP->add(dummy.in(range(0, 0)));
    ...
    auto obj=(product(c1,Pg) + product(c2,pow(Pg,2)) + sum(c0))*(1e-3)/upper_bound;
    SDP->min(obj);
    ...
    if(!grid._tree && grid.add_3d_nlin && sdp_cuts && solv_type==ipopt)
    {
 ...
 //        SDPOPF->add_real(I_from.in(arcs)==0);
         ...
         SDP->add_real(I_from.in(arcs_curr)==0);
         ...
         SDP->add(I_from_Pf.in(arcs_curr)==0, true, "on/off", false);
         ...
         SDP->add_real(I_to.in(arcs_curr)==0);
         ...
         SDP->add(I_to_Pf.in(arcs_curr)==0, true, "on/off", false);
    }
...
//                    int c=res1[i][j];
//                    for(auto it=SDP->_vars.begin();it!=SDP->_vars.end();it++){
//                        if(it->first>=c){
//                        if(it->first>c){
//                            it--;
//                        }
//                            auto v= SDP->get_var<double>(it->second->_name);
//                            cut+=v(c-it->first)*res1[i][j+1];
//                            break;
//                        }
//                    }
//                    DebugOff(res1[i][j+1]<<" ");
...
//        }
        if(true){
        auto res=SDP->cuts_eigen_bags_primal_complex(1e-6, "Wii", "R_Wij", "Im_Wij");
        if(res.size()>=1){
        ...
        SDP->reindex();
        solver<> s(SDP,solv_type);
        s.run(output = 5 , 1e-6, "ma57");
    }
//    else
//    SDPOPF.run();
    auto SDPa=build_SDPOPF(grid, false, true, true);
    SDPa->print();
