...
int main (int argc, char * argv[]) {
    ...
    if(!linearize){
        ...
        auto res=OPF->run_obbt(SDP,max_time,max_iter,nb_threads=4,ub_solver_type,lb_solver_type, ub_solver_tol, lb_solver_tol, range_tol);
        ...
        auto SDP= build_SDPOPF(grid, current, nonlin_obj, sdp_kim);
        auto res=OPF->run_obbt(SDP,max_time,max_iter,nb_threads=4,ub_solver_type,lb_solver_type, ub_solver_tol, lb_solver_tol, range_tol, true);
        lower_bound = SDP->get_obj_val();
        lower_bound_init = get<3>(res);
        total_iter=get<1>(res);
        total_time=get<2>(res);
