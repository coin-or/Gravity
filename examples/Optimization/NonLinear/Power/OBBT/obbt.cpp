...
if(linearize){
foutp<<"$nref^{U}_{batch}$= "<<nb_refine<<", $nref^{U}_{root}$= "<<nb_root_refine<<", $nb^I_{root}$ "<<nb_root_ref_init<<" $viol^{I}_{batch}$= "<< viol_obbt_init<<", $viol^{I}_{root}$= "<<viol_root_init<<", linearize= "<<linearize<<", lag "<<lag<<endl;
}
else{
