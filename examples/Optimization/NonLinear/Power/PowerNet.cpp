...
int PowerNet::readgrid(const string& fname, bool reverse_arcs) {
    ...
    while(word.compare("];")) {
        ...
        ch_half_sq.add_val(name, arc->ch*0.5/(sqrt(pow(arc->g,2)+pow(arc->b,2))));
        g_sq.add_val(name, arc->g/(sqrt(pow(arc->g,2)+pow(arc->b,2))));
        b_sq.add_val(name, arc->b/(sqrt(pow(arc->g,2)+pow(arc->b,2))));
        ...
        lij_max.add_val(name, pow(arc->tr,2)*pow(arc->limit, 2)/(pow(bus_s->vbound.min,2)*sqrt(pow(arc->b,2)+pow(arc->g,2))));
        lji_min.add_val(name, 0);
        lji_max.add_val(name, pow(arc->limit, 2)/(pow(bus_d->vbound.min,2)*sqrt(pow(arc->b,2)+pow(arc->g,2))));
...
shared_ptr<Model<>> build_SDPOPF_QC(PowerNet& grid, bool loss, double upper_bound, double lower_bound)
{
    ...
    if(add_original){
        ...
        if(!grid._tree)
        {
            ...
            //            Debug("size "<<ref_node_pairs_ijkl.first.first.size());
            //
...
shared_ptr<Model<>> build_SDPOPF(PowerNet& grid, bool current, bool nonlin_obj, bool sdp_kim, double upper_bound, bool sdp_cuts)
{
    bool relax,  llnc=false, lazy_bool = false, add_original=false, convexify=true;
    size_t num_bags = 0;
    string num_bags_s = "100";
    num_bags = atoi(num_bags_s.c_str());
    ...
    auto qf_to_min=grid.qf_to_min.in(arcs);
    auto qf_to_max=grid.qf_to_max.in(arcs);
    auto lij_min=grid.lij_min.in(arcs_curr);
    auto lij_max=grid.lij_max.in(arcs_curr);
    auto lji_min=grid.lji_min.in(arcs_curr);
    auto lji_max=grid.lji_max.in(arcs_curr);
    auto Iij_min=grid.Iij_min.in(arcs);
    auto Iij_max=grid.Iij_max.in(arcs);
    auto c1 = grid.c1.in(gens);
    auto c2 = grid.c2.in(gens);
    ...
    auto b_tf = grid.b_tf.in(arcs);
    auto b_tt = grid.b_tt.in(arcs);
    auto g_sq=grid.g_sq.in(arcs);
    auto b_sq=grid.b_sq.in(arcs);
    auto S_max = grid.S_max.in(arcs);
    auto v_max = grid.v_max.in(nodes);
    auto v_min = grid.v_min.in(nodes);
    auto w_max = grid.w_max.in(nodes);
    ...
    var<> lji("lji", lji_min,lji_max);
    if(current){
        SDPOPF->add(lij.in(arcs_curr));
        SDPOPF->add(lji.in(arcs_curr));
    }
...
//        SDPOPF->add_real(I_from.in(arcs)==0);
        ...
        //SDPOPF->add(I_from_Pf.in(arcs_curr)<=0);
        ...
        //SDPOPF->add(I_to_Pf.in(arcs_curr)<=0);
...
shared_ptr<Model<>> build_ACOTS(PowerNet& grid, PowerModelType pmt, int output, double tol){
    ...
    Model<> OTS("AC-OTS Model");
    /** Variables */
    /* Power generation variables */
    var<> Pg("Pg", pg_min, pg_max);
    var<> Qg ("Qg", qg_min, qg_max);
    OTS.add(Pg.in(gens),Qg.in(gens));
//    Pg.copy_vals(grid.pg_s);
//    Pg.initialize_av();
//    Qg.initialize_uniform();
    /* Power flow variables */
    ...
    var<> Qf_to("Qf_to", -1.*S_max,S_max);
    var<int> y("y", 0, 1);
    OTS.add(Pf_from.in(arcs), Qf_from.in(arcs),Pf_to.in(arcs),Qf_to.in(arcs));
    OTS.add(y.in(arcs));
...
//        vr.initialize_uniform(0.99,1.01);
    ...
    OTS.add(KCL_P.in(nodes) == 0);
    OTS.add(KCL_Q.in(nodes) == 0);
    ...
    Constraint<> P_From_one_a("P_From_one_a");
    P_From_one_a=Flow_P_From.in(arcs)-Pf_from.in(arcs)-1E6*(1-y.in(arcs));
    OTS.add(P_From_one_a.in(arcs)<=0);
    
    Constraint<> P_From_one_b("P_From_one_b");
    P_From_one_b=Flow_P_From.in(arcs)*(-1)+Pf_from.in(arcs)-1E6*(1-y.in(arcs));
    OTS.add(P_From_one_b<=0);
    
    Constraint<> P_From_zero_a("P_From_zero_a");
    P_From_zero_a=Pf_from-S_max*y;
    OTS.add(P_From_zero_a.in(arcs)<=0);
    
    Constraint<> P_From_zero_b("P_From_zero_b");
    P_From_zero_b=(-1)*S_max*y-Pf_from;
    OTS.add(P_From_zero_b.in(arcs)<=0);
    ...
    Constraint<> P_To_one_a("P_To_one_a");
    P_To_one_a=Flow_P_To.in(arcs)-Pf_to.in(arcs)-1E6*(1-y.in(arcs));
    OTS.add(P_To_one_a.in(arcs)<=0);
                                                             
    Constraint<> P_To_one_b("P_To_one_b");
    P_To_one_b=Flow_P_To.in(arcs)*(-1)+Pf_to.in(arcs)-1E6*(1-y.in(arcs));
    OTS.add(P_To_one_b<=0);
                                                             
    Constraint<> P_To_zero_a("P_To_zero_a");
    P_To_zero_a=Pf_to-S_max*y;
    OTS.add(P_To_zero_a.in(arcs)<=0);
                                                             
    Constraint<> P_To_zero_b("P_To_zero_b");
    P_To_zero_b=(-1)*S_max*y-Pf_to;
    OTS.add(P_To_zero_b.in(arcs)<=0);
   ...
   Constraint<> Q_From_one_a("Q_From_one_a");
   Q_From_one_a=Flow_Q_From.in(arcs)-Qf_from.in(arcs)-1E6*(1-y.in(arcs));
   OTS.add(Q_From_one_a.in(arcs)<=0);
   
   Constraint<> Q_From_one_b("Q_From_one_b");
   Q_From_one_b=Flow_Q_From.in(arcs)*(-1)+Qf_from.in(arcs)-1E6*(1-y.in(arcs));
   OTS.add(Q_From_one_b<=0);
   
   Constraint<> Q_From_zero_a("Q_From_zero_a");
   Q_From_zero_a=Qf_from-S_max*y;
   OTS.add(Q_From_zero_a.in(arcs)<=0);
   
   Constraint<> Q_From_zero_b("Q_From_zero_b");
   Q_From_zero_b=(-1)*S_max*y-Qf_from;
   OTS.add(Q_From_zero_b.in(arcs)<=0);
   ...
   Constraint<> Q_To_one_a("Q_To_one_a");
   Q_To_one_a=Flow_Q_To.in(arcs)-Qf_to.in(arcs)-1E6*(1-y.in(arcs));
   OTS.add(Q_To_one_a.in(arcs)<=0);
                                           
   Constraint<> Q_To_one_b("Q_To_one_b");
   Q_To_one_b=Flow_Q_To.in(arcs)*(-1)+Qf_to.in(arcs)-1E6*(1-y.in(arcs));
   OTS.add(Q_To_one_b.in(arcs)<=0);
                                                           
   Constraint<> Q_To_zero_a("Q_To_zero_a");
   Q_To_zero_a=Qf_to-S_max*y;
   OTS.add(Q_To_zero_a.in(arcs)<=0);
                                                           
   Constraint<> Q_To_zero_b("Q_To_zero_b");
   Q_To_zero_b=(-1)*S_max*y-Qf_to;
   OTS.add(Q_To_zero_b.in(arcs)<=0);
    ...
    if (!polar) {
        ...
        Vol_limit_UB = pow(vr, 2) + pow(vi, 2);
        Vol_limit_UB -= pow(v_max, 2);
        OTS.add(Vol_limit_UB.in(nodes) <= 0);
        
        Constraint<> Vol_limit_LB("Vol_limit_LB");
        Vol_limit_LB = pow(vr, 2) + pow(vi, 2);
        Vol_limit_LB -= pow(v_min,2);
        OTS.add(Vol_limit_LB.in(nodes) >= 0);
    }
    ...
    Constraint<> PAD_LB("PAD_LB");
    if (polar) {
        PAD_UB = theta.from(arcs) - theta.to(node_pairs);
        PAD_UB -= th_max.in_pairs(arcs);
        PAD_LB = theta.from(arcs) - theta.to(arcs);
        PAD_LB -= th_min.in_pairs(arcs);
    }
    else {
        DebugOff("Number of node_pairs = " << node_pairs.size() << endl);
        PAD_UB = vi.from(arcs)*vr.to(arcs) - vr.from(arcs)*vi.to(arcs);
        PAD_UB -= tan_th_max.in_pairs(arcs)*(vr.from(arcs)*vr.to(arcs) + vi.from(arcs)*vi.to(arcs));

        PAD_LB = vi.from(arcs)*vr.to(arcs) - vr.from(arcs)*vi.to(arcs);
        PAD_LB -= tan_th_min.in_pairs(arcs)*(vr.from(arcs)*vr.to(arcs) + vi.from(arcs)*vi.to(arcs));
    }
        OTS.add(PAD_UB.in(arcs) <= 0);
        OTS.add(PAD_LB.in(arcs) >= 0);
    ...
    Thermal_Limit_from += pow(Pf_from, 2) + pow(Qf_from, 2);
    Thermal_Limit_from -= pow(S_max, 2);
    OTS.add(Thermal_Limit_from.in(arcs) <= 0);
    
    Constraint<> Thermal_Limit_to("Thermal_Limit_to");
    Thermal_Limit_to += pow(Pf_to, 2) + pow(Qf_to, 2);
    Thermal_Limit_to -= pow(S_max,2);
    OTS.add(Thermal_Limit_to.in(arcs) <= 0);
    
    Constraint<> fix_y("fix_y");
    fix_y=y-1;
    //OTS.add(fix_y.in(arcs) == 0);
    
    OTS.print();
//    OTS.initialize_uniform();
    solver<> OPF(OTS,ipopt);
     ...
     */
    /** Terminal output */
    string out = "DATA_OPF, " + grid._name + ", " + to_string(nb_buses) + ", " + to_string(nb_lines) +", " + to_string_with_precision(OTS.get_obj_val(),10) + ", " + to_string(-numeric_limits<double>::infinity()) + ", " + to_string(solve_time) + ", LocalOptimal, " + to_string(total_time);
    DebugOn(out <<endl);
    
    OTS.print_solution();
    
    OTS.print_constraints_stats(1e-6);
}
//shared_ptr<Model<>> build_SDPOTS(PowerNet& grid)
//{
//    bool relax,  llnc=false, lazy_bool = false, add_original=false, convexify=true, sdp_cuts=false;
//    size_t num_bags = 0;
//    string num_bags_s = "100";
//    num_bags = atoi(num_bags_s.c_str());
//
