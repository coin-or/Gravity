...
{
public:
    pair<double,double> roll, pitch, yaw;
    double ub = numeric_limits<double>::max(), ub_ = numeric_limits<double>::max(),lb = 0;
    int depth;
    gravity::indices valid_cells;
    gravity::param<double> dist_cost_cells;
    bool leaf;
    friend bool operator < (const struct treenode_r & n1, const struct treenode_r & n2)
    {
        if(n1.lb != n2.lb)
            return n1.lb > n2.lb;
        else
    ...
    treenode_r(const pair<double,double>& roll, const pair<double,double>& pitch, const pair<double,double>& yaw, double lb, double ub, double ub_, int depth, const gravity::indices& valid_cells, bool leaf, gravity::param<double>& dist_cost_cells):roll(roll),pitch(pitch),yaw(yaw),lb(lb),ub(ub),ub_(ub_), depth(depth), valid_cells(valid_cells), leaf(leaf), dist_cost_cells(dist_cost_cells){};
};
class treenode_m
{
public:
    ...
    gravity::indices valid_cells;
    bool leaf;
    friend bool operator < (const struct treenode_m & n1, const struct treenode_m & n2)
    {
        if(n1.lb != n2.lb)
            return n1.lb > n2.lb;
        else
    ...
    treenode_m(const pair<double,double>& roll, const pair<double,double>& pitch, const pair<double,double>& yaw, const pair<double,double>& tx,const pair<double,double>& ty,const pair<double,double>& tz, double lb, double ub, double ub_, int depth, const gravity::indices& valid_cells, bool leaf):roll(roll),pitch(pitch),yaw(yaw),tx(tx),ty(ty),tz(tz),lb(lb),ub(ub),ub_(ub_), depth(depth), valid_cells(valid_cells), leaf(leaf){};
};
class treenode_n
{
public:
    ...
    friend bool operator < (const struct treenode_n & n1, const struct treenode_n & n2)
    {
        if(n1.lb != n2.lb)
            return n1.lb > n2.lb;
        else
