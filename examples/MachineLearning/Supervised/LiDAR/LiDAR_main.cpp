...
vector<double> BranchBound4(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max);

vector<double> BranchBound4_ICP(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max);

vector<double> BranchBound5(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max);
...
indices preprocess_poltyope_intersect_opt(const vector<vector<double>>& point_cloud_data, const vector<vector<double>>& point_cloud_model, const indices& old_cells, double roll_min, double roll_max, double pitch_min, double pitch_max, double yaw_min, double yaw_max, double shift_min_x, double shift_max_x, double shift_min_y, double shift_max_y, double shift_min_z, double shift_max_z, const vector<vector<vector<double>>>& model_voronoi_normals, const vector<vector<double>>& model_face_intercept, const vector<vector<vector<double>>>& model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, double upper_bound, int nb_total_threads);
...
shared_ptr<Model<double>> build_linobj_convex(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, const indices& valid_cells, double new_roll_min, double new_roll_max, double new_pitch_min, double new_pitch_max, double new_yaw_min, double new_yaw_max, double new_shift_min_x, double new_shift_max_x, double new_shift_min_y, double new_shift_max_y, double new_shift_min_z, double new_shift_max_z, vector<double>& rot_trans, bool separate, const vector<pair<pair<int,int>,pair<int,int>>>& incompatibles,  param<>& norm_x,  param<>& norm_y,  param<>& norm_z,  param<>& intercept,const vector<int>& init_matching, const vector<double>& error_per_point, const vector<double>& model_inner_prod_min, const vector<double>& model_inner_prod_max, bool relax_inits, double ub){
    ...
    param<> rot_x_min("rot_x_min");
    param<> rot_x_max("rot_x_max");
    param<> rot_y_min("rot_y_min");
    param<> rot_y_max("rot_y_max");
    param<> rot_z_min("rot_z_min");
    param<> rot_z_max("rot_z_max");
    ...
    for(auto i=0;i<nd;i++){
        ...
        rot_x_min.add_val(to_string(i+1), rx_lb[i]);
        rot_x_max.add_val(to_string(i+1), rx_ub[i]);
        rot_y_min.add_val(to_string(i+1), ry_lb[i]);
        rot_y_max.add_val(to_string(i+1), ry_ub[i]);
        rot_z_min.add_val(to_string(i+1), rz_lb[i]);
        rot_z_max.add_val(to_string(i+1), rz_ub[i]);
    ...
    var<> rotx("rotx", rot_x_min, rot_x_max), roty("roty", rot_y_min, rot_y_max), rotz("rotz", rot_z_min, rot_z_max);
    //var<> rotx("rotx"), roty("roty"), rotz("rotz");
    ...
    Constraint<> Def_newxm("Def_newxm");
    Def_newxm = new_xm-product(x2.in(ids),bin.in_matrix(1, 1));
    Def_newxm += rotx+x_shift;
    Reg->add(Def_newxm.in(N1)==0);
    
    Constraint<> Def_newym("Def_newym");
    Def_newym = new_ym-product(y2.in(ids),bin.in_matrix(1, 1));
    Def_newym += roty+y_shift;
    Reg->add(Def_newym.in(N1)==0);
    
    Constraint<> Def_newzm("Def_newzm");
    Def_newzm = new_zm-product(z2.in(ids),bin.in_matrix(1, 1));
    Def_newzm += rotz+z_shift;
    Reg->add(Def_newzm.in(N1)==0);
    ...
    if(hybrid){
        ...
        sum_roty=sum(roty_shifty);
        //Reg->add(sum_roty==0);
        
        Constraint<> sum_rotz("sum_rotz");
        sum_rotz=sum(rotz_shiftz);
        //Reg->add(sum_rotz==0);
        ...
        Constraint<> dist_model("dist_model");
        dist_model=xm_min_i*xm_max_i+ym_min_i*ym_max_i+zm_min_i*zm_max_i+product(dm.in(ids),bin.in_matrix(1, 1))-new_xm*(xm_min_i+xm_max_i)-new_ym*(ym_min_i+ym_max_i)-new_zm*(zm_min_i+zm_max_i);
        // Reg->add(dist_model.in(N1)<=0);
...
std::pair<int,double> build_linobj_ipopt(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, const indices& valid_cells, double new_roll_min, double new_roll_max, double new_pitch_min, double new_pitch_max, double new_yaw_min, double new_yaw_max, double new_shift_min_x, double new_shift_max_x, double new_shift_min_y, double new_shift_max_y, double new_shift_min_z, double new_shift_max_z, vector<double>& rot_trans, bool separate, const vector<pair<pair<int,int>,pair<int,int>>>& incompatibles,  param<>& norm_x,  param<>& norm_y,  param<>& norm_z,  param<>& intercept,const vector<int>& init_matching, const vector<double>& error_per_point, bool relax_inits, double ub){
    ...
    //var<> new_x1("new_x1"),new_y1("new_y1"),new_z1("new_z1");
    var<> deltax("deltax", 0, 4), deltay("deltay", 0, 4), deltaz("deltaz", 0, 4);
    if(hybrid){
        //var<> deltax("deltax", deltax_lb, deltax_ub), deltay("deltay", deltay_lb, deltay_ub), deltaz("deltaz", deltaz_lb, deltaz_ub);
        ...
        Constraint<> Def_deltax("Def_deltax");
        Def_deltax=pow(new_xm-new_x1, 2)-deltax;
        Reg->add(Def_deltax.in(N1)<=0);
        
        Constraint<> Def_deltay("Def_deltay");
        Def_deltay=pow(new_ym-new_y1, 2)-deltay;
        Reg->add(Def_deltay.in(N1)<=0);
        
        Constraint<> Def_deltaz("Def_deltaz");
        Def_deltaz=pow(new_zm-new_z1, 2)-deltaz;
        Reg->add(Def_deltaz.in(N1)<=0);
        ...
        if(hybrid){
            ...
            param<> dm_cells("dm_cells");
            dm_cells.in(cells);
            param<> xm_min_i("xm_min_i");param<> xm_max_i("xm_max_i");
            param<> ym_min_i("ym_min_i");param<> ym_max_i("ym_max_i");
            param<> zm_min_i("zm_min_i");param<> zm_max_i("zm_max_i");
            vector<vector<vector<double>>> box;
            vector<vector<double>> box_i;
            vector<double> coord_i;
            coord_i.resize(3);
            ...
            //            Constraint<> dist_model("dist_model");
            //            dist_model=xm_min*xm_max+ym_min*ym_max+zm_min*zm_max+product(dm.in(ids),bin.in_matrix(1, 1))-new_xm*(xm_min+xm_max)-new_ym*(ym_min+ym_max)-new_zm*(zm_min+zm_max);
            //             Reg->add(dist_model.in(N1)<=0);
        ...
        for(auto i=0;i<nd;i++){
            ...
            *new_z1_bounds = {x_range->first + y_range->first + z_range->first + shift_min_z,
            ...
            for (int j = 0; j<nm; j++) {
                ...
                auto voro_model=voro_model1->copy();
                auto normals=model_voronoi_normals.at(j);
                auto intercepts=model_face_intercept.at(j);
                indices voronoi_ids("voronoi_ids");
                voronoi_ids = range(1,intercepts.size());
                param<> a("a"), b("b"), c("c"), d("d");
                a.in(voronoi_ids);b.in(voronoi_ids);c.in(voronoi_ids);d.in(voronoi_ids);
                for(auto k=0;k<intercepts.size();k++){
                    a.set_val(k, normals[k][0]);
                    b.set_val(k, normals[k][1]);
                    c.set_val(k, normals[k][2]);
                    d.set_val(k, intercepts[k]);
                }
                ...
                if(voro_model->_status==0){
                    ...
                    if(dist_vj_max>=sphere_inner_sq[i]-1e-6){
                        voro_model2->min(pow((x-point_cloud_model.at(j)[0]),2)+pow((y-point_cloud_model.at(j)[1]),2)+pow((z-point_cloud_model.at(j)[2]),2));
                        //voro_model2->print();
                        ...
                        if(voro_model2->_status==-1|| (voro_model2->_status==0 && voro_model2->get_obj_val()-1e-6<=upper_bound)){
                            ...
                            DebugOn("res.first "<<voro_model2->get_obj_val()<<" upper_bound "<<upper_bound<<endl);
                        }
                    }
                    else{
            ...
            for(auto k=0;k<inf.size();k++){
                ...
                for(auto l=0;l<edge_set_k.size();l++){
                    ...
                    if(std::find (inf.begin(), inf.end(), elf)==inf.end()||std::find (inf.begin(), inf.end(), els)==inf.end()){
                        ...
                        DebugOn("Edge set is faulty "<<endl);
                    }
                }
            }
            if(option_cost_new){
                ...
                if(!res){
                    ...
                    if(dist_vj_max>=sphere_inner_sq[i]-1e-6){
                        ...
                        if(res.first-1e-6<=upper_bound){
                            ...
                            DebugOn("res.first "<<res.first<<" upper_bound "<<upper_bound<<endl);
                        }
                    }
                    else{
                        DebugOff("infeasible cell"<<endl);
...
/*Min and maximum distance squared to given centre ((x-cx)^2+(y-cy)^2+(z-cz)^2) is computed over a box X with a plane cutting the box. Each vertex corresponds to either exteme points of the box due to bounds on x,y,z or extrme points formed by intersection of the box with a plane.
 @coords: list of all extreme points of polytope
 @new_coords: list of extreme points(subset of coords) formed by intersection of box with plane
    ...
    if(a*xs+b*ys+c*zs+intercept<=1e-6){
        ...
        if(!min_found){
            ...
            if(abs(yl+yu)>=1e-10 || abs(zl+zu)>=1e-10){
                ...
                auto lamda_2=2.0*(intercept-xl*(xl+xu)-cy*(yl+yu)-cz*(zl+zu))/((yl+yu)*(yl+yu)+(zl+zu)*(zl+zu));
                if(lamda_2>=-1e-10){
                    xs=xl;
            ...
            if(abs(xl+xu)>=1e-10 || abs(zl+zu)>=1e-10){
                ...
                auto lamda_4=2.0*(intercept-cx*(xl+xu)-yl*(yl+yu)-cz*(zl+zu))/((xl+xu)*(xl+xu)+(zl+zu)*(zl+zu));
                if(lamda_4>=-1e-10){
                    ys=yl;
            ...
            if(abs(xl+xu)>=1e-10 || abs(yl+yu)>=1e-10){
                ...
                auto lamda_6=2.0*(intercept-cx*(xl+xu)-cy*(yl+yu)-zl*(zl+zu))/((xl+xu)*(xl+xu)+(yl+yu)*(yl+yu));
                if(lamda_6>=-1e-10){
                    zs=zl;
...
vector<double> BranchBound2(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max) {
    ...
    for (int i = 0; i<nb_threads; i++) {
        ...
        if(valid_cells[i].size()!=0){
            auto model = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells[i], roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, rot_trans[i], false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point, model_inner_prod_min,model_inner_prod_max, false, best_ub);
            model->write();
            models.push_back(model);
        }
    }
...
vector<double> BranchBound4_ICP(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max)
{
    ...
    vector<pair<double,double>> shift_x_bounds_new, shift_y_bounds_new, shift_z_bounds_new;
    vector<pair<double,double>> roll_bounds_new, pitch_bounds_new, yaw_bounds_new;
    vector<indices> valid_cells_new;
    vector<vector<double>> rot_trans(nb_threads, vector<double>(12));
    ...
    auto valid_cells_ro=indices(N1,N2);
    models_new.resize(0);
    shift_x_bounds_new.resize(0);
    shift_y_bounds_new.resize(0);
    shift_z_bounds_new.resize(0);
    roll_bounds_new.resize(0);
    pitch_bounds_new.resize(0);
    ...
    while (elapsed_time < total_time_max && lb_queue.top().lb<=best_ub && !lb_queue.empty() && opt_gap > max_opt_gap) {
        ...
        while(topnode.lb<=best_ub) {
            ...
            auto vi = preprocess_poltyope_intersect(point_cloud_data, point_cloud_model, topnode.valid_cells, roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, model_voronoi_normals, model_face_intercept,model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, 0, nb_threads);
...
//                }
                                        ...
                                        if(ub<=best_ub+1e-2){
                                            ...
                                            DebugOn(rot_trans_ub[6]<<" "<<rot_trans_ub[7]<<" "<<rot_trans_ub[8]<<endl);
                                            DebugOn(rot_trans_ub[9]<<" "<<rot_trans_ub[10]<<" "<<rot_trans_ub[11]<<endl);
...
vector<double> BranchBound6(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max) {
    ...
    depth_vec_new.resize(0);
    pos_vec_new.resize(0);
    valid_cells_new.resize(0);
    auto goicp=initialize_ICP_only(point_cloud_model, point_cloud_data);
    auto valid_cells_r = preprocess_poltyope_intersect_new(point_cloud_data, point_cloud_model, valid_cells_ro, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, model_voronoi_normals, model_face_intercept, model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, best_ub, nb_threads);
    auto model = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, rot_trans_r, false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point, model_inner_prod_min, model_inner_prod_max, false, best_ub);
    //auto model=build_polyhedral_ipopt(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max,pitch_min,pitch_max,  yaw_min, yaw_max,  shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z,  rot_trans_r, false,  incompatible_pairs,  norm_x, norm_y,  norm_z,  intercept, init_matching,  init_err_per_point,  model_voronoi_normals,  model_face_intercept,  false);
    lb_queue.push(treenode_n(model, roll_bounds_r, pitch_bounds_r, yaw_bounds_r, shift_x_bounds_r, shift_y_bounds_r, shift_z_bounds_r, lb, ub, ub_, depth_r, valid_cells_r));
    treenode_n topnode=lb_queue.top();
    ...
    while (elapsed_time < total_time_max && lb_queue.top().lb<=best_ub && !lb_queue.empty() && opt_gap > max_opt_gap) {
        ...
        depth_vec.clear();
//        models=models_new;
//        shift_x_bounds=shift_x_bounds_new;
//        shift_y_bounds=shift_y_bounds_new;
//        shift_z_bounds=shift_z_bounds_new;
//        roll_bounds=roll_bounds_new;
//        pitch_bounds=pitch_bounds_new;
//        yaw_bounds=yaw_bounds_new;
//        pos_vec=pos_vec_new;
//        depth_vec=depth_vec_new;
//        valid_cells=valid_cells_new;
//        pos_vec_new.clear();
//        models_new.clear();
//        shift_x_bounds_new.clear();
//        shift_y_bounds_new.clear();
//        shift_z_bounds_new.clear();
//        roll_bounds_new.clear();
//        pitch_bounds_new.clear();
...
//        depth_vec_new.clear();
        ...
        while(topnode.lb<=best_ub) {
            ...
            auto vi = preprocess_poltyope_intersect_new(point_cloud_data, point_cloud_model, topnode.valid_cells, roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, model_voronoi_normals, model_face_intercept,model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, best_ub, nb_threads);
            valid_cells.push_back(vi);
            depth_vec.push_back(topnode.depth+1);
            DebugOn(vi.size()<<endl);
            if(valid_cells[i].size()>=1){
                auto m = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells[i], roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, rot_trans_temp, false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point,  model_inner_prod_min, model_inner_prod_max, false, best_ub);
                //auto m=build_polyhedral_ipopt(point_cloud_model, point_cloud_data, valid_cells[i], roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, rot_trans[i], false,  incompatible_pairs,  norm_x, norm_y,  norm_z,  intercept, init_matching,  init_err_per_point,  model_voronoi_normals,  model_face_intercept,  false);
                if(branch1){
                    initialize_model_from_parent(topnode.mod, m);
...
//                lb_queue.push(treenode_n(model, roll_bounds[i],  pitch_bounds[i], yaw_bounds[i], shift_x_bounds[i], shift_y_bounds[i], shift_z_bounds[i], topnode.lb, best_ub, -1.0, topnode.depth+1, valid_cells[i]));
            ...
            auto vi1 = preprocess_poltyope_intersect_new(point_cloud_data, point_cloud_model, topnode.valid_cells, roll_bounds[i+1].first, roll_bounds[i+1].second,  pitch_bounds[i+1].first, pitch_bounds[i+1].second, yaw_bounds[i+1].first, yaw_bounds[i+1].second, shift_x_bounds[i+1].first, shift_x_bounds[i+1].second, shift_y_bounds[i+1].first, shift_y_bounds[i+1].second, shift_z_bounds[i+1].first, shift_z_bounds[i+1].second, model_voronoi_normals, model_face_intercept,model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, best_ub, nb_threads);
            valid_cells.push_back(vi1);
            depth_vec.push_back(topnode.depth+1);
...
//            }
            ...
            DebugOn("models.s "<<models.size()<<endl);
            DebugOn("validc.s "<<valid_cells.size()<<endl);
            DebugOn("shiftx.s "<<shift_x_bounds.size()<<endl);
            DebugOn("i "<<i<<endl);
            DebugOn("yaw.s "<<yaw_bounds.size()<<endl);
        }
        elapsed_time = get_wall_time() - time_start;
        DebugOn("Elapsed time = " << elapsed_time << "seconds\n");
        if(elapsed_time + max_time > total_time_max){
...
vector<double> BranchBound7(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max){
    ...
    valid_cells_new.resize(0);
    auto goicp=initialize_ICP_only(point_cloud_model, point_cloud_data);
    auto valid_cells_r = preprocess_poltyope_intersect_opt(point_cloud_data, point_cloud_model, valid_cells_ro, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, model_voronoi_normals, model_face_intercept, model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, best_ub, nb_threads);
    auto model = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, rot_trans_r, false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point, model_inner_prod_min, model_inner_prod_max, false, best_ub);
    //auto model=build_polyhedral_ipopt(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max,pitch_min,pitch_max,  yaw_min, yaw_max,  shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z,  rot_trans_r, false,  incompatible_pairs,  norm_x, norm_y,  norm_z,  intercept, init_matching,  init_err_per_point,  model_voronoi_normals,  model_face_intercept,  false);
    lb_queue.push(treenode_n(model, roll_bounds_r, pitch_bounds_r, yaw_bounds_r, shift_x_bounds_r, shift_y_bounds_r, shift_z_bounds_r, lb, ub, ub_, depth_r, valid_cells_r));
    treenode_n topnode=lb_queue.top();
...
//        depth_vec_new.clear();
        ...
        while(topnode.lb<=best_ub) {
            ...
            auto vi = preprocess_poltyope_intersect_opt(point_cloud_data, point_cloud_model, topnode.valid_cells, roll_bounds[i].first, roll_bounds[i].second,  pitch_bounds[i].first, pitch_bounds[i].second, yaw_bounds[i].first, yaw_bounds[i].second, shift_x_bounds[i].first, shift_x_bounds[i].second, shift_y_bounds[i].first, shift_y_bounds[i].second, shift_z_bounds[i].first, shift_z_bounds[i].second, model_voronoi_normals, model_face_intercept,model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, best_ub, nb_threads);
            valid_cells.push_back(vi);
            depth_vec.push_back(topnode.depth+1);
            DebugOn(vi.size()<<endl);
            if(valid_cells[i].size()>=1){
...
vector<double> BranchBound5(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, param<>& norm_x, param<>& norm_y, param<>& norm_z,  param<>& intercept, const vector<int>& init_matching, const vector<double>& init_err_per_point, param<>& model_radius, vector<vector<vector<double>>> model_voronoi_normals, vector<vector<double>> model_face_intercept, vector<vector<vector<double>>> model_voronoi_vertices, vector<int>& new_model_pts, indices& new_model_ids, param<>& dist_cost, bool relax_ints, bool relax_sdp, bool rigid_transf, const vector<double>& model_inner_prod_min,const vector<double>& model_inner_prod_max) {
    ...
    auto valid_cells_ro=indices(N1,N2);
    auto valid_cells_r = preprocess_poltyope_intersect(point_cloud_data, point_cloud_model, valid_cells_ro, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, model_voronoi_normals, model_face_intercept, model_voronoi_vertices, new_model_pts, new_model_ids, dist_cost, 0, nb_threads);
    auto model = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max, pitch_min, pitch_max, yaw_min, yaw_max, shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z, rot_trans_r, false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point, model_inner_prod_min, model_inner_prod_max, false, 12);
    //auto model=build_polyhedral_ipopt(point_cloud_model, point_cloud_data, valid_cells_r, roll_min, roll_max,pitch_min,pitch_max,  yaw_min, yaw_max,  shift_min_x, shift_max_x, shift_min_y, shift_max_y, shift_min_z, shift_max_z,  rot_trans_r, false,  incompatible_pairs,  norm_x, norm_y,  norm_z,  intercept, init_matching,  init_err_per_point,  model_voronoi_normals,  model_face_intercept,  false);
    ...
    while (elapsed_time < total_time_max && !lb_queue.empty() && opt_gap > max_opt_gap) {
        ...
        for (int i = 0; i<nb_threads; i+=2) {
            ...
            if((valid_cells[i+1].size()!=0)){
                auto m = build_linobj_convex(point_cloud_model, point_cloud_data, valid_cells[i+1], roll_bounds[i+1].first, roll_bounds[i+1].second,  pitch_bounds[i+1].first, pitch_bounds[i+1].second, yaw_bounds[i+1].first, yaw_bounds[i+1].second, shift_x_bounds[i+1].first, shift_x_bounds[i+1].second, shift_y_bounds[i+1].first, shift_y_bounds[i+1].second, shift_z_bounds[i+1].first, shift_z_bounds[i+1].second, rot_trans[i+1], false, incompatible_pairs, norm_x, norm_y, norm_z, intercept,init_matching, init_err_per_point, model_inner_prod_min, model_inner_prod_max, false, best_ub);
                //auto m=build_polyhedral_ipopt(point_cloud_model, point_cloud_data, valid_cells[i+1], roll_bounds[i+1].first, roll_bounds[i+1].second,  pitch_bounds[i+1].first, pitch_bounds[i+1].second, yaw_bounds[i+1].first, yaw_bounds[i+1].second, shift_x_bounds[i+1].first, shift_x_bounds[i+1].second, shift_y_bounds[i+1].first, shift_y_bounds[i+1].second, shift_z_bounds[i+1].first, shift_z_bounds[i+1].second, rot_trans[i+1], false,  incompatible_pairs,  norm_x, norm_y,  norm_z,  intercept, init_matching,  init_err_per_point,  model_voronoi_normals,  model_face_intercept,  false);
                if(topnode.ub_>0 && max_ratio<1){
                    initialize_model_from_parent(topnode.mod, m);
                }
