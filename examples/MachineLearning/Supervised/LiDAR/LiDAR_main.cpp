...
int main (int argc, char * argv[])
{
    ...
    if(Registration){
        ...
        if(ARMO || IPH || run_goICP){
            ...
            auto SOC_MIP = build_linobj_convex(ext_model, ext_data, rot_trans,new_matching, separate=false, incompatibles, norm_x, norm_y, norm_z, intercept,min_max_model, matching, false, cells);
            //SOC_MIP->print();
...
shared_ptr<Model<double>> build_linobj_convex(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, vector<double>& rot_trans, vector<int>& new_matching, bool separate, const vector<pair<pair<int,int>,pair<int,int>>>& incompatibles, param<>& norm_x, param<>& norm_y, param<>& norm_z, param<>& intercept, const vector<pair<double,double>>& min_max_model, const vector<int>& matching, bool relax_ints, indices cells){
    double roll_1 = 0, yaw_1 = 0, pitch_1 = 0;
    int nb_pairs = 0, min_nb_pairs = numeric_limits<int>::max(), max_nb_pairs = 0, av_nb_pairs = 0;
    size_t nm = point_cloud_model.size(), nd = point_cloud_data.size();
    vector<double> zeros = {0,0,0};
