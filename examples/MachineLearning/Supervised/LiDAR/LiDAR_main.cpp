...
//            }
        ...
        for (const auto &frame: frames1) {
            ...
            for (const auto &p: *frame.second->_lidar_points) {
                ...
                old_x_vec1.push_back(p->_x);
                x_shift_all1.push_back(frame.second->_uav_point->_x);
                old_y_vec1.push_back(p->_y);
                y_shift_all1.push_back(frame.second->_uav_point->_y);
                old_z_vec1.push_back(p->_z);
                z_shift_all1.push_back(frame.second->_uav_point->_height);
                i++;
            }
        ...
        for (const auto &frame: frames2) {
            ...
            for (auto const &p: *frame.second->_lidar_points) {
                ...
                old_x_vec2.push_back(p->_x);
                x_shift_all2.push_back(frame.second->_uav_point->_x);
                old_y_vec2.push_back(p->_y);
                y_shift_all2.push_back(frame.second->_uav_point->_y);
                old_z_vec2.push_back(p->_z);
                z_shift_all2.push_back(frame.second->_uav_point->_height);
                i++;
            }
        }
        if(frames1.size()!=0)
        ...
        if(!bypass){
            ...
            //        return 0;
            bool solve_lidar_cube = false, solve_lidar_iter = true;
            int m = av_nb_pairs;
            //            int m = 1;
...
//                var<> yaw1("yaw1", 0, 0), pitch1("pitch1", 0, 0), roll1("roll1", 0, 0);
        ...
        if(save_file){
            ...
            laswriteopener.set_file_name("manual.laz");
