...
vector<double> ub_heuristic_disc(vector<vector<double>>& point_cloud_model, vector<vector<double>>& point_cloud_data, double roll_min, double roll_max, double pitch_min, double pitch_max, double yaw_min, double yaw_max, double tx_min, double tx_max, double ty_min, double ty_max, double tz_min, double tz_max, vector<double>& best_rot_trans, double& best_ub, std::string error_type, double max_time=100)
{
    ...
    while(!stop){
        ...
        run_ub_parallel( point_cloud_model, point_cloud_data,  roll_lb,   roll_ub,   pitch_lb,   pitch_ub,  yaw_lb,  yaw_ub, tx_lb,  tx_ub,  ty_lb,  ty_ub, tz_lb,  tz_ub, roll_min,  roll_max, pitch_min,  pitch_max,  yaw_min,  yaw_max, tx_min,  tx_max, ty_min,ty_max, tz_min,  tz_max,  error_type, ub_node, nb_threads);
        roll_lb.clear();
        roll_ub.clear();
        pitch_lb.clear();
        pitch_ub.clear();
