...
shared_ptr<Model<double>> Align_L2_model_rotation_orthogonal(const vector<vector<double>>& point_cloud_model, const vector<vector<double>>& point_cloud_data, const vector<vector<double>>& uav_model, const vector<vector<double>>& uav_data, const vector<vector<double>>& rollpitchyawins_model, const vector<vector<double>>& rollpitchyawins_data, double roll_min, double roll_max, double pitch_min, double pitch_max, double yaw_min, double yaw_max, indices& cells, param<double> dist_cost)
{
    ...
    minor1_Def=theta22*theta33-theta13*theta32-minor1;
...
shared_ptr<Model<double>> Align_L2_model_rotation_neworder_scanner(const vector<vector<double>>& point_cloud_model, const vector<vector<double>>& point_cloud_data, const vector<vector<double>>& uav_model, const vector<vector<double>>& uav_data, const vector<vector<double>>& rollpitchyawins_model, const vector<vector<double>>& rollpitchyawins_data, double roll_min, double roll_max, double pitch_min, double pitch_max, double yaw_min, double yaw_max, indices& cells, param<double> dist_cost)
{
    double angle_max = 0.1;
    int nb_pairs = 0, min_nb_pairs = numeric_limits<int>::max(), max_nb_pairs = 0, av_nb_pairs = 0;
...
// param<> sx("sx"), sy("sy"), sz("sz");
    
    var<> sx("sx", -0.2, 0.2), sy("sy", -0.2, 0.2), sz("sz", -0.2, 0.2);
    Reg->add(sx.in(R(1)),sy.in(R(1)),sz.in(R(1)));
    ...
    var<> xb_d("xb_d"), yb_d("yb_d"), zb_d("zb_d");
    Reg->add(xb_d.in(N1), yb_d.in(N1), zb_d.in(N1));
    var<> xs_d("xs_d"), ys_d("ys_d"), zs_d("zs_d");
    Reg->add(xs_d.in(N1), ys_d.in(N1), zs_d.in(N1));
    var<> xb_m("xb_m"), yb_m("yb_m"), zb_m("zb_m");
    Reg->add(xb_m.in(N2), yb_m.in(N2), zb_m.in(N2));
    var<> xs_m("xs_m"), ys_m("ys_m"), zs_m("zs_m");
    Reg->add(xs_m.in(N1), ys_m.in(N1), zs_m.in(N1));
    
    auto ids1 = theta11.repeat_id(N1.size());
    
    Constraint<> xd_bore("xd_bore");
    xd_bore=(x1i-sx.in(ids1))*theta11.in(ids1)+(y1i-sy.in(ids1))*theta12.in(ids1)+(z1i-sz.in(ids1))*theta13.in(ids1)-xb_d;
    Reg->add(xd_bore.in(N1)==0);
    
    Constraint<> yd_bore("yd_bore");
    yd_bore=(x1i-sx.in(ids1))*theta21.in(ids1)+(y1i-sy.in(ids1))*theta22.in(ids1)+(z1i-sz.in(ids1))*theta23.in(ids1)-yb_d;
    Reg->add(yd_bore.in(N1)==0);

    Constraint<> zd_bore("zd_bore");
    zd_bore=(x1i-sx.in(ids1))*theta31.in(ids1)+(y1i-sy.in(ids1))*theta32.in(ids1)+(z1i-sz.in(ids1))*theta33.in(ids1)-zb_d;
    Reg->add(zd_bore.in(N1)==0);

    Constraint<> xs_ins_inv1("xs_ins_inv1");
    xs_ins_inv1=sx.in(ids1)*cos(roll_ins1)*cos(yaw_ins1)+sy.in(ids1)*(cos(pitch_ins1)*sin(yaw_ins1) +cos(yaw_ins1)*sin(roll_ins1)*sin(pitch_ins1))+sz.in(ids1)*(sin(pitch_ins1)*sin(yaw_ins1)-cos(pitch_ins1)*cos(yaw_ins1)*sin(roll_ins1))-xs_d;
    Reg->add(xs_ins_inv1.in(N1)==0);
    
    Constraint<> ys_ins_inv1("ys_ins_inv1");
    ys_ins_inv1=sx.in(ids1)*(-1)*cos(roll_ins1)*sin(yaw_ins1)+sy.in(ids1)*(cos(pitch_ins1)*cos(yaw_ins1) -sin(roll_ins1)*sin(pitch_ins1)*sin(yaw_ins1))+sz.in(ids1)*(cos(yaw_ins1)*sin(pitch_ins1)+cos(pitch_ins1)*sin(roll_ins1)*sin(yaw_ins1))-ys_d;
    Reg->add(ys_ins_inv1.in(N1)==0);
    
    Constraint<> zs_ins_inv1("zs_ins_inv1");
    zs_ins_inv1=sx.in(ids1)*sin(roll_ins1)+sy.in(ids1)*(-1)*cos(roll_ins1)*sin(pitch_ins1)+sz.in(ids1)*(cos(roll_ins1)*cos(pitch_ins1))-zs_d;
    Reg->add(zs_ins_inv1.in(N1)==0);
   
    
    Constraint<> xd_ins_inv("xd_ins_inv");
    xd_ins_inv=xb_d*cos(roll_ins1)*cos(yaw_ins1)+yb_d*(cos(pitch_ins1)*sin(yaw_ins1) +cos(yaw_ins1)*sin(roll_ins1)*sin(pitch_ins1))+zb_d*(sin(pitch_ins1)*sin(yaw_ins1)-cos(pitch_ins1)*cos(yaw_ins1)*sin(roll_ins1))+x_uav1+xs_d-new_xm-x_diff;
    Reg->add(xd_ins_inv.in(N1)==0);
    
    Constraint<> yd_ins_inv("yd_ins_inv");
    yd_ins_inv=xb_d*(-1)*cos(roll_ins1)*sin(yaw_ins1)+yb_d*(cos(pitch_ins1)*cos(yaw_ins1) -sin(roll_ins1)*sin(pitch_ins1)*sin(yaw_ins1))+zb_d*(cos(yaw_ins1)*sin(pitch_ins1)+cos(pitch_ins1)*sin(roll_ins1)*sin(yaw_ins1))+y_uav1+ys_d-new_ym-y_diff;
    Reg->add(yd_ins_inv.in(N1)==0);
    
    Constraint<> zd_ins_inv("zd_ins_inv");
    zd_ins_inv=xb_d*sin(roll_ins1)+yb_d*(-1)*cos(roll_ins1)*sin(pitch_ins1)+zb_d*(cos(roll_ins1)*cos(pitch_ins1))+z_uav1+zs_d-new_zm-z_diff;
    Reg->add(zd_ins_inv.in(N1)==0);
 
    auto ids2 = theta11.repeat_id(N2.size());
    
    Constraint<> xm_bore("xm_bore");
    xm_bore=(x2i-sx.in(ids2))*theta11.in(ids2)+(y2i-sy.in(ids2))*theta12.in(ids2)+(z2i-sz.in(ids2))*theta13.in(ids2)-xb_m;
    Reg->add(xm_bore.in(N2)==0);
    
    Constraint<> ym_bore("ym_bore");
    ym_bore=(x2i-sx.in(ids2))*theta21.in(ids2)+(y2i-sy.in(ids2))*theta22.in(ids2)+(z2i-sz.in(ids2))*theta23.in(ids2)-yb_m;
    Reg->add(ym_bore.in(N2)==0);

    Constraint<> zm_bore("zm_bore");
    zm_bore=(x2i-sx.in(ids2))*theta31.in(ids2)+(y2i-sy.in(ids2))*theta32.in(ids2)+(z2i-sz.in(ids2))*theta33.in(ids2)-zb_m;
    Reg->add(zm_bore.in(N2)==0);

    Constraint<> xs_ins_inv2("xs_ins_inv2");
    xs_ins_inv2=sx.in(ids2)*cos(roll_ins2)*cos(yaw_ins2)+sy.in(ids2)*(cos(pitch_ins2)*sin(yaw_ins2) +cos(yaw_ins2)*sin(roll_ins2)*sin(pitch_ins2))+sz.in(ids2)*(sin(pitch_ins2)*sin(yaw_ins2)-cos(pitch_ins2)*cos(yaw_ins2)*sin(roll_ins2))-xs_m;
    Reg->add(xs_ins_inv2.in(N2)==0);
        
    Constraint<> ys_ins_inv2("ys_ins_inv2");
    ys_ins_inv2=sx.in(ids2)*(-1)*cos(roll_ins2)*sin(yaw_ins2)+sy.in(ids2)*(cos(pitch_ins2)*cos(yaw_ins2) -sin(roll_ins2)*sin(pitch_ins2)*sin(yaw_ins2))+sz.in(ids2)*(cos(yaw_ins2)*sin(pitch_ins2)+cos(pitch_ins2)*sin(roll_ins2)*sin(yaw_ins2))-ys_m;
    Reg->add(ys_ins_inv2.in(N2)==0);
    
    Constraint<> zs_ins_inv2("zs_ins_inv2");
    zs_ins_inv2=sx.in(ids2)*sin(roll_ins2)+sy.in(ids2)*(-1)*cos(roll_ins2)*sin(pitch_ins2)+sz.in(ids2)*(cos(roll_ins2)*cos(pitch_ins2))-zs_m;
    Reg->add(zs_ins_inv2.in(N2)==0);

    Constraint<> xm_ins_inv("xm_ins_inv");
    xm_ins_inv=xb_m*cos(roll_ins2)*cos(yaw_ins2)+yb_m*(cos(pitch_ins2)*sin(yaw_ins2) +cos(yaw_ins2)*sin(roll_ins2)*sin(pitch_ins2))+zb_m*(sin(pitch_ins2)*sin(yaw_ins2)-cos(pitch_ins2)*cos(yaw_ins2)*sin(roll_ins2))+x_uav2+xs_m-xm;
    Reg->add(xm_ins_inv.in(N2)==0);
        
    Constraint<> ym_ins_inv("ym_ins_inv");
    ym_ins_inv=xb_m*(-1)*cos(roll_ins2)*sin(yaw_ins2)+yb_m*(cos(pitch_ins2)*cos(yaw_ins2) -sin(roll_ins2)*sin(pitch_ins2)*sin(yaw_ins2))+zb_m*(cos(yaw_ins2)*sin(pitch_ins2)+cos(pitch_ins2)*sin(roll_ins2)*sin(yaw_ins2))+y_uav2+ys_m-ym;
    Reg->add(ym_ins_inv.in(N2)==0);
    
    Constraint<> zm_ins_inv("zm_ins_inv");
    zm_ins_inv=xb_m*sin(roll_ins2)+yb_m*(-1)*cos(roll_ins2)*sin(pitch_ins2)+zb_m*(cos(roll_ins2)*cos(pitch_ins2))+z_uav2+zs_m-zm;
    Reg->add(zm_ins_inv.in(N2)==0);
  ...
  //  Reg->min(sum(pow(x_diff,2) + pow(y_diff,2) + pow(z_diff,2)));
    ...
    S.run();
    DebugOn("sx "<<endl);
    DebugOn(sx.eval()<<" "<<sy.eval()<<" "<<sz.eval());
    return Reg;
}
#endif /* Lower_Bound_h */
