...
template <typename Numeric>
    void plot3(const std::vector<Numeric> &uav_x,
               ...
               const std::vector<Numeric> &uav_z, const std::vector<Numeric> &x,
           const std::vector<Numeric> &y,
           const std::vector<Numeric> &z,const std::vector<Numeric> &a,
               const std::vector<Numeric> &b,
               const std::vector<Numeric> &c,           const std::map<std::string, std::string> &keywords =
           std::map<std::string, std::string>())
{
    // Same as with plot_surface: We lazily load the modules here the first time
    // this function is called because I'm not sure that we can assume "matplotlib
    ...
    PyObject *xarray2 = detail::get_array(a);
    PyObject *yarray2 = detail::get_array(b);
    PyObject *zarray2 = detail::get_array(c);
    ...
    PyTuple_SetItem(args2, 0, xarray2);
    PyTuple_SetItem(args2, 1, yarray2);
    PyTuple_SetItem(args2, 2, zarray2);
    ...
    PyObject *res1 = PyObject_Call(plot4, args1, kwargs);
    if (!res1) throw std::runtime_error("Failed 3D line plot");
    PyObject *plot5 = PyObject_GetAttrString(axis, "plot");
    if (!plot5) throw std::runtime_error("No 3D line plot");
    Py_INCREF(plot5);
    PyObject *res2 = PyObject_Call(plot5, args2, kwargs);
    if (!res2) throw std::runtime_error("Failed 3D line plot");
    Py_DECREF(plot3);
    Py_DECREF(plot4);
    Py_DECREF(plot5);
    Py_DECREF(axis);
...
template <typename Numeric>
    void plot3(const std::vector<Numeric> &uav_x,
           ...
           const std::vector<Numeric> &z,const std::vector<Numeric> &a,
               const std::vector<Numeric> &b,
               const std::vector<Numeric> &c,  const std::vector<Numeric> &d,
               const std::vector<Numeric> &e,
               const std::vector<Numeric> &f,           const std::map<std::string, std::string> &keywords =
           std::map<std::string, std::string>())
{
    // Same as with plot_surface: We lazily load the modules here the first time
    // this function is called because I'm not sure that we can assume "matplotlib
    ...
    PyObject *xarray3 = detail::get_array(d);
    PyObject *yarray3 = detail::get_array(e);
    PyObject *zarray3 = detail::get_array(f);
    ...
    PyObject *args3 = PyTuple_New(3);
    PyTuple_SetItem(args3, 0, xarray3);
    PyTuple_SetItem(args3, 1, yarray3);
    PyTuple_SetItem(args3, 2, zarray3);
    ...
    PyObject *res2 = PyObject_Call(plot5, args2, kwargs);
    if (!res2) throw std::runtime_error("Failed 3D line plot");
    PyObject *plot6 = PyObject_GetAttrString(axis, "plot");
    if (!plot6) throw std::runtime_error("No 3D line plot");
    Py_INCREF(plot6);
    PyObject *res3 = PyObject_Call(plot6, args3, kwargs);
    if (!res3) throw std::runtime_error("Failed 3D line plot");
    Py_DECREF(plot3);
    Py_DECREF(plot4);
    Py_DECREF(plot5);
    Py_DECREF(plot6);
...
//    Py_INCREF(gca_kwargs);
    ...
    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "s", PyLong_FromLong(1));
//    PyDict_SetItemString(kwargs, "s", PyString_FromString("0.1"));
//    PyDict_SetItemString(kwargs, "color", PyString_FromString("grey"));
//    PyDict_SetItemString(kwargs, "edgecolor", PyString_FromString("r"));
//    Py_INCREF(kwargs);
    for (const auto& it : keywords)
    {
        PyDict_SetItemString(kwargs, it.first.c_str(), PyString_FromString(it.second.c_str()));
    }
//    map<NumericZ,vector<pair<NumericX,NumericY>>> map_by_z;
    DebugOn("Number of points = " << z.size() << endl);
    NumericZ max_z = 0;
    vector<NumericX> xi, xj;
    vector<NumericY> yi, yj;
    vector<NumericZ> zi, zj;
    for (auto i = 0; i<z.size(); i++) {
//        map_by_z[z[i]].push_back(make_pair(x[i], y[i]));
        if(max_z < z[i])
            max_z = z[i];
    }
    for (auto i = 0; i<z.size(); i++) {
    ...
    DebugOn("Number of red points = " << zi.size() << endl);
//    for (auto &level: map_by_z){
    
//        vector<NumericX> xi;
//        vector<NumericY> yi;
//        vector<NumericZ> zi;
//        for (const auto &xy: level.second){
//            xi.push_back(xy.first);
//            yi.push_back(xy.second);
//            zi.push_back(level.first);
//        }
    PyObject* kwargsi = PyDict_New();
//    Py_INCREF(kwargsi);
    PyDict_SetItemString(kwargsi, "s", PyLong_FromLong(1));
    //    PyDict_SetItemString(kwargs, "s", PyString_FromString("0.1"));
    PyDict_SetItemString(kwargsi, "color", PyString_FromString("red"));
    
    PyObject* xarray = detail::get_array(xj);
    PyObject* yarray = detail::get_array(yj);
    PyObject* zarray = detail::get_array(zj);
...
//    Py_INCREF(res);
    ...
    PyObject* xarrayi = detail::get_array(xi);
    PyObject* yarrayi = detail::get_array(yi);
    PyObject* zarrayi = detail::get_array(zi);

//    Py_INCREF(xarrayi);
//    Py_INCREF(yarrayi);
//    Py_INCREF(zarrayi);

    PyObject* plot_argsi = PyTuple_New(3);
    PyTuple_SetItem(plot_argsi, 0, xarrayi);
    PyTuple_SetItem(plot_argsi, 1, yarrayi);
    PyTuple_SetItem(plot_argsi, 2, zarrayi);

//    Py_INCREF(plot_argsi);


//    PyObject *figi =
//    PyObject_CallObject(detail::_interpreter::get().s_python_function_figure,
//                        detail::_interpreter::get().s_python_empty_tuple);
//    if (!figi) throw std::runtime_error("Call to figure() failed.");
//    Py_INCREF(figi);
    PyObject *gca_kwargsi = PyDict_New();
    PyDict_SetItemString(gca_kwargsi, "projection", PyString_FromString("3d"));
    //    Py_INCREF(gca_kwargs);
    PyObject *gcai = PyObject_GetAttrString(fig, "gca");
    if (!gca) throw std::runtime_error("No gca");
    Py_INCREF(gcai);
    PyObject *axisi = PyObject_Call(
                                   gcai, detail::_interpreter::get().s_python_empty_tuple, gca_kwargsi);
    
    if (!axisi) throw std::runtime_error("No axis");
    Py_INCREF(axisi);

    PyObject *plot3i = PyObject_GetAttrString(axisi, "scatter");
    if (!plot3i) throw std::runtime_error("No 3D scatter plot");
    Py_INCREF(plot3i);
    //    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_scatter, plot_args, kwargs);
    PyObject *resi = PyObject_Call(plot3i, plot_argsi, kwargsi);
//    Py_INCREF(resi);
    if (!resi) throw std::runtime_error("Failed 3D scatter plot");
...
//    Py_DECREF(yarray);
//    Py_DECREF(zarray);
    if(resi) Py_DECREF(resi);
    if(res) Py_DECREF(res);
    Py_DECREF(plot3i);
    Py_DECREF(kwargs);
    Py_DECREF(kwargsi);
    Py_DECREF(plot_argsi);
//    Py_DECREF(xarrayi);
//    Py_DECREF(yarrayi);
//    Py_DECREF(zarrayi);
////    }
