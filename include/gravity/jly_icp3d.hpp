...
public:
	...
	T Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t);
	T Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, size_t max_iter);
	T Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, T err_diff);
	T Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, size_t max_iter, T err_diff);
...
template <typename T>
T ICP3D<T>::Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t)
{
	return Run(data, n, R, t, max_iter_def, err_diff_def);
}

template <typename T>
T ICP3D<T>::Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, size_t max_iter)
{
	return Run(data, n, R, t, max_iter, err_diff_def);
}

template <typename T>
T ICP3D<T>::Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, T err_diff)
{
	return Run(data, n, R,  t, max_iter_def, err_diff);
}

template <typename T>
T ICP3D<T>::Run(T * data, size_t n, Go_ICP::Matrix & R, Go_ICP::Matrix & t, size_t max_iter, T err_diff)
{
  size_t num;
	...
	Go_ICP::Matrix p_d(num,3); // data

	// init mean
	Go_ICP::Matrix mu_m(1,3);
	Go_ICP::Matrix mu_d(1,3);
	...
	for(iter = 0; iter < max_iter; iter++)
	{
		...
		Go_ICP::Matrix U,W,V;
		H.svd(U,W,V);
		Go_ICP::Matrix R_ = V*~U;

		//There are some problems with Go_ICP::Matrix::det(), so it is not used
		//R11*(R22*R33-R23*R32)
