        ...
        int run(int output, type tol , int max_iter, double mipgap, bool relax, pair<bool,string> lin_solver, double time_limit ){
            ...
            while(violated_constraints && optimal){
                if (_stype==ipopt) {
                    ...
                    iapp->Options()->SetNumericValue("bound_relax_factor", tol*0.9);
...
//                    _model->_first_run = false;
    ...
    int run_parallel_new(const std::vector<std::string> objective_models, std::vector<double>& sol_obj, std::vector<int>& sol_status, std::vector<std::vector<double>>& sol_val, const std::vector<shared_ptr<gravity::Model<double>>>& models, gravity::SolverType stype=ipopt, double tol=1e-6, unsigned nr_threads=std::thread::hardware_concurrency(), const string& lin_solver="", int max_iter=1e6, bool linearize=false);
