...
namespace gravity {
    ...
    public:
        ...
        bool cuts_MPI(vector<shared_ptr<Model<>>> batch_models, int batch_model_count, const Model<type>& interior_model, shared_ptr<Model<>> lin, int& oacuts, double active_tol, int run_obbt_iter, map<string, bool>& fixed_point, double range_tol, vector<string> objective_models, vector<int> sol_status);
        void reset_lazy();
        int num_obbt_prob(){
            int count=0;
            for(auto &v:_vars){
