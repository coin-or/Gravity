...
//                }
        ...
        if(add_Kim_Kojima || add_SDP_3d || determinant_level>1){/* Need to extend Wij indices to include new chordal edges*/
            ...
            for(auto bag:bags_3d){
                ...
                int i_3 = stoi(id3);
                if(i_2<i_1){
                    swap(name_v1,name_v2);
                    swap(id1,id2);
                    swap(i_1,i_2);
                }
                if(i_3<i_2){
                    swap(name_v3,name_v2);
                    swap(id3,id2);
                    swap(i_3,i_2);
                }
                if(i_2<i_1){
...
//                else
        ...
        for( const auto & iter: quad_eq_sparsity){
            ...
            if(con_vec.size()>0){
                ...
                vector<param<>> y_coefs(nb_int_lin_terms);/* coefficient multiplying each symbolic integer variable */
                vector<pair<indices,indices>> xx_ids(nb_cont_quad_terms);/* indices for each symbolic continuous variable appearing in continuous quadratic terms */
                vector<pair<indices,indices>> yy_ids(nb_int_quad_terms);/* indices for each symbolic integer variable appearing in integer quadratic terms */
                vector<pair<indices,indices>> xy_ids(nb_hyb_quad_terms);/* indices for each symbolic integer variable appearing in continuous*integer quadratic terms */
                vector<param<>> xx_coefs(nb_cont_quad_terms);/* coefficient multiplying each symbolic continuous variable appearing in continuous quadratic terms */
                vector<param<>> yy_coefs(nb_int_quad_terms);/* coefficient multiplying each symbolic integer variable appearing in integer quadratic terms */
                vector<param<>> xy_coefs(nb_hyb_quad_terms);/* coefficient multiplying each symbolic integer variable appearing in continuous*integer quadratic terms */
...
//                    c.deep_copy(*c_pair.second);
    ...
    vector<pair<shared_ptr<var<double>>,shared_ptr<func<>>>> project() {/*<  Use the equations where at least one variable appears linearly to express it as a function of other variables in the problem */
        ...
        while(!eq_list.empty()){
            ...
            for (auto& lterm: c->get_lterms()) {
                auto v = lterm.second._p;
                if(v->get_intype()==double_ && !c->in_quad_part(v) /* only appears in linear part of c*/ && c->appear_once_linear(v) /* only appears once in linear part of c*/&& !lterm.second._coef->has_zero() /* (does not include zero, i.e., is invertible */ && v->_indices->has_unique_ids()  && *v->_in/* && !_obj->has_sym_var(*v)  && !is_nonlinear(*v) && !_obj->has_sym_var(*v)v does not appear in polynomial or nonlinear constraints */){
            ...
            if(vv.is_matrix_indexed()){
                ...
                if(v->_indices->nb_keys()==0){
                    c->delete_lterm(*v);/* Remove coef*v from c */
                }
                else {
                ...
                f.delete_lterm(*lterm._p);/* Remove coef*v from f */
            }
            if (lterm._coef->is_function()) {
                auto coef = *static_pointer_cast<func<T>>(lterm._coef);
                
                if(coef._range->first==coef._range->second){
                    f *= -1./coef.eval();
                }
                else {
...
//            MC4 <= 0;
    ...
    template<typename T=type,
    typename std::enable_if<is_same<T,double>::value>::type* = nullptr>
    std::tuple<bool,int,double,double,double,double,double,double,int,int,int> run_obbt_one_iteration(shared_ptr<Model<T>> relaxed_model= nullptr, double max_time = 1000, unsigned max_iter=1e3, double rel_tol=1e-2, double abs_tol=1e6, unsigned nb_threads = 1, SolverType ub_solver_type = ipopt, SolverType lb_solver_type = ipopt, double ub_solver_tol=1e-6, double lb_solver_tol=1e-6, double range_tol=1e-3, shared_ptr<Model<T>> obbt_model= nullptr, Model<T> & interior_model=nullptr, bool linearize=false, bool lag=false, int run_obbt_iter=1, double lb_scale_value=1e6, double solver_time_start=0, int nb_refine=1, int nb_root_refine=1, int nb_root_ref_init=1, double viol_obbt_init=0.1, double viol_root_init=0.1, bool initialize_primal=false, double upper_bound_best=1e10);
