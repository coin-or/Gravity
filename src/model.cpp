...
//        this->print();
        ...
        if(this->_status==0)
        {
            ...
            gapnl=std::abs(upper_bound.second-lower_bound_init)/std::abs(upper_bound.second)*100;
            DebugOn("Initial gap "<<gapnl<<endl);
            double lower_bound=lower_bound_init;
            

            
          if (std::abs(upper_bound.second-lower_bound)>=abs_tol || std::abs(upper_bound.second-lower_bound)/(std::abs(upper_bound.second)+zero_tol)>=rel_tol)
        {
            terminate=false;
            ...
            {    solver_time= get_wall_time()-solver_time_start;
               ...
               // SDPLB1.run(output = 5, tol, "ma57");
                ...
                if(this->_status==0)
                {
                    ...
                    auto gap = 100*std::abs(upper_bound.second - lower_bound)/std::abs(upper_bound.second);
