...
ConfigMap::~ConfigMap() {
  ...
  for (std::list<double*>::iterator iter = allocated_memory_collector.begin();
...
double * ConfigMap::getVector(char * key_) {
  ...
  double * return_vector = new double[st.numberOfTokens()];
  allocated_memory_collector.push_back(return_vector);

  for (int i = 0; st.numberOfTokens() > 0; i++) {
    return_vector[i] = (double)atof(st.nextToken().c_str());
  }
  
  return return_vector;
};

double * ConfigMap::getVector(const char * key_) {
